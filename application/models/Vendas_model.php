...
class Vendas_model extends CI_Model
{
    ...
    function get($table, $fields, $where = '', $id_usuario, $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
        ...
        if ($where) {
            ...
            $this->db->where('vendas.status = 1 AND vendas.id_usuario = ' . $id_usuario);
        }
    ...
    public function autoCompleteProduto($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row['precoVenda'] = number_format($row['precoVenda'], 2, ',', '.');
    ...
    public function autoCompleteCliente($q, $id_usuario)
    {
